...
- 補上 `eslint`（目前 scripts 有 lint 指令但未安裝 eslint）
- 為 API utils 加最小單元測試（auth/token、keys sanitize）
- 規劃去除 `prompt/alert/confirm`，改一致 Modal UX（可另開一輪）
- 將 `R2` 相關操作抽 service 層，進一步降低 route 體積
